...
function extractJobData() {
	...
	return {
		jobTitle: getText(titleParagraph),
		companyName: getText(company),
		location: queryText(metadataParagraph, "span"),
		jobDescription: queryText(aboutJob, '[data-testid="expandable-text-box"]'),
		companyDescription: getText(companyDescriptionBox),
		tags: [...chipsRow.querySelectorAll('a[href*="/jobs/"]')].map(getText).filter(x => !!x),
		url: `https://www.linkedin.com/jobs/view/${aboutJob.id.slice(aboutJobIdPrefix.length)}/`,
		companyTags: [...(companyDescriptionBox?.parentElement.previousElementSibling?.children ?? [])].map(getText).filter(x => x && x !== "•"),
	};
}
...
function queryText(root, selector) {
	return getText(query(root, selector));
}
